/**
 * Shared validation core. SINGLE SOURCE OF TRUTH for TEKKAL deck schema
 * and quality checks. Two wrappers consume this module:
 *
 *   - src/ai/validation.ts — in-app validateDeck for the AI tool loop,
 *     plus imperative helpers (buildFixInstructions, resolveOverlaps)
 *     that are NOT part of the schema check itself.
 *
 *   - scripts/tekkal-validate.mjs — standalone CLI validator, kept in
 *     plain JavaScript so it can be copied into user project folders
 *     without a toolchain. It MUST mirror the checks here verbatim;
 *     drift tests in src/ai/tekkal-validate.test.ts catch regressions.
 *
 * Keep this file TypeScript-typed but framework-free: no React,
 * no Zustand, no async I/O. Anything that needs a side effect
 * (patching the deck, writing files, spawning dialogs) belongs in the
 * caller, not here.
 */
 ...
 * this in lockstep.
 */
export const VALID_ASSET_PATH_RE = /^(\.\/assets\/|\/assets\/|https?:\/\/|data:)/;
 ...
 */
export function effectiveSize(size: unknown): { w: number | undefined; h: number | undefined } {
...
export function normHex(c: unknown): string | null {
...
export function collectStyleColors(style: unknown, out: Set<string>): void {
